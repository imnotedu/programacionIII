...
async function getOrCreateCart(userId) {
    ...
    const res = await (0, database_1.query)('SELECT * FROM carts WHERE userId = $1', [userId]);
    let cart = res.rows[0];
    if (!cart) {
        // Crear nuevo carrito
        ...
        const newCartRes = await (0, database_1.query)('SELECT * FROM carts WHERE id = $1', [cartId]);
...
async function addItem(req, res, next) {
    try {
        ...
        const existingItemRes = await (0, database_1.query)(`
        ...
        if (existingItem) {
          ...
          `, [newQuantity, now, existingItem.id]);
            ...
            await (0, database_1.query)('UPDATE carts SET updatedAt = $1 WHERE id = $2', [now, cart.id]);
            const updatedItemRes = await (0, database_1.query)('SELECT * FROM cart_items WHERE id = $1', [existingItem.id]);
            res.json({
                success: true,
          ...
          `, [itemId, cart.id, data.productId, data.quantity, now, now]);
            // Actualizar timestamp del carrito
            await (0, database_1.query)('UPDATE carts SET updatedAt = $1 WHERE id = $2', [now, cart.id]);
            const newItemRes = await (0, database_1.query)('SELECT * FROM cart_items WHERE id = $1', [itemId]);
            res.status(201).json({
                success: true,
...
async function updateItem(req, res, next) {
    try {
        ...
        // Actualizar timestamp del carrito
        await (0, database_1.query)('UPDATE carts SET updatedAt = $1 WHERE id = $2', [now, cart.id]);
        const updatedItemRes = await (0, database_1.query)('SELECT * FROM cart_items WHERE id = $1', [item.id]);
        res.json({
            success: true,
            data: { item: updatedItemRes.rows[0] },
...
async function removeItem(req, res, next) {
    try {
        ...
        // Eliminar item
        await (0, database_1.query)('DELETE FROM cart_items WHERE id = $1', [item.id]);
        // Actualizar timestamp del carrito
        const now = new Date().toISOString();
        await (0, database_1.query)('UPDATE carts SET updatedAt = $1 WHERE id = $2', [now, cart.id]);
        res.json({
...
async function clearCart(req, res, next) {
    try {
        ...
        await (0, database_1.query)('DELETE FROM cart_items WHERE cartId = $1', [cart.id]);
        // Actualizar timestamp del carrito
        const now = new Date().toISOString();
        await (0, database_1.query)('UPDATE carts SET updatedAt = $1 WHERE id = $2', [now, cart.id]);
        res.json({
...
async function checkout(req, res, next) {
    const client = await (0, database_1.getClient)(); // Obtener cliente dedicado para transacción
    try {
        const userId = req.user.userId;
        ...
        const items = itemsRes.rows;
        if (items.length === 0) {
            throw new errors_1.ConflictError('El carrito está vacío');
        }
        // Validar stock antes de procesar
        for (const item of items) {
            if (item.quantity > item.product_stock) {
                throw new errors_1.ConflictError(`Stock insuficiente para "${item.product_name}". Disponible: ${item.product_stock}, solicitado: ${item.quantity}`);
            }
        }
        // INICIAR TRANSACCIÓN
        ...
        for (const item of items) {
            ...
            if (currentStock < item.quantity) {
                throw new errors_1.ConflictError(`Stock insuficiente para producto ${item.product_name} durante el procesamiento.`);
            }
            await client.query(`
              UPDATE products 
