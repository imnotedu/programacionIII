...
class ViewController {
    ...
    async home(req, res) {
        try {
            const { data: response } = await axios_1.default.get(`${API_BASE}/products`);
    ...
    async store(req, res) {
        try {
            ...
            if (category && typeof category === 'string' && category.trim()) {
                products = products.filter((p) => p.category.toLowerCase() === category.toLowerCase().trim());
            }
    ...
    async productDetail(req, res) {
        try {
            ...
            const { data: response } = await axios_1.default.get(`${API_BASE}/products/${id}`);
    ...
    async cart(req, res) {
        try {
            ...
            if (productIds.length > 0) {
                ...
                const productPromises = productIds.map(id => axios_1.default.get(`${API_BASE}/products/${id}`).catch(() => null));
    ...
    async favorites(req, res) {
        try {
            ...
            if (favoriteIds.length > 0) {
                // Obtener productos uno por uno
                const productPromises = favoriteIds.map(id => axios_1.default.get(`${API_BASE}/products/${id}`).catch(() => null));
                const responses = await Promise.all(productPromises);
                products = responses
                    .filter(res => res !== null)
                    .map(res => res.data.data.product);
